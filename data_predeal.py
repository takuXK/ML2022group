...
def MultiCloudPointRandomSample(dataPath, allFile=True, startFile=0, numFile=1024, numSample=1024,
    ...
    if allFile:
        ...
        if numFile < len(os.listdir(dataPath)):
            fileList = os.listdir(dataPath)[startFile: startFile + numFile]
        else:
...
def IOUcalculation(confusionMatrix):
    IOU_list  = []
    confusionMatrix = confusionMatrix.tolist()
    for i in range(len(confusionMatrix)):
        intersection = confusionMatrix[i][i]
        unionset = np.sum(confusionMatrix[:][i]) + np.sum(confusionMatrix[i][:]) - intersection
        IOU = intersection / unionset
        IOU_list.append(IOU)
    miou = np.mean(IOU_list[:-1])
...
if __name__ == "__main__":
    ...
    resultData2 = MultiCloudPointRandomSample(dataPath, False, startFile=0, numFile=30, numSample=1024, savefile=True, savefilePath=savefilePath)
    """

    # train data path
    train_ratio = 0.8
    train_num = int(train_ratio * len(fileList))
    savetrainPath = 'F:\ZJU课程\机器学习（胡浩基）\FinalAssignment\RepeatPointNet\\trainpath\\'
    resultData1 = MultiCloudPointRandomSample(dataPath, False, startFile=0, numFile=train_num, numSample=1024, savefile=True, savefilePath=savetrainPath)
    # validation data path
    val_num = len(fileList) - train_num
    savevalPath = 'F:\ZJU课程\机器学习（胡浩基）\FinalAssignment\RepeatPointNet\\valpath\\'
    resultData2 = MultiCloudPointRandomSample(dataPath, False, startFile=train_num, numFile=val_num, numSample=1024, savefile=True,savefilePath=savevalPath)